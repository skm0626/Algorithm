...
dgim_2 = DGIM_2()
dgim_result_2=[0 for i in range(2001)]
real_result_2=[0 for i in range(2001)]

for b in stream:
    dgim_2.put(b)
    # print(dgim.bucket_tower)
for k in range (1,2001):
    dgim_result_2[k]=dgim_2.count(k)
    real_result_2[k]=sum(stream[-k:]) # 앞은 예측값, 뒤는 실제값

print(real_result[-1], sum(real[-2000:]), dgim_result[-1], dgim_result_2[-1])
...
plt.subplot(413)
plt.title("Method 2")
plt.plot([i for i in range(2001)], dgim_result_2,'g-')
plt.subplot(414)
plt.title("Real sum, Method 1, Method 2 comparison")
plt.plot([i for i in range(2001)], real_result, [i for i in range(2001)], dgim_result,'r-', [i for i in range(2001)], dgim_result_2,'g-')
plt.show()